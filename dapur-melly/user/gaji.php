...
if (!$query_karyawan || mysqli_num_rows($query_karyawan) == 0) {
    die("<div style='padding: 20px; text-align: center;'><h3>Data karyawan tidak ditemukan</h3><p>Hubungi admin untuk verifikasi akun.</p></div>");
}
$data_karyawan = mysqli_fetch_assoc($query_karyawan);

$id_karyawan = $data_karyawan['id_karyawan'];
$gaji_pokok = $data_karyawan['gaji_pokok'];
$posisi_karyawan = $data_karyawan['posisi'] ?? '';
